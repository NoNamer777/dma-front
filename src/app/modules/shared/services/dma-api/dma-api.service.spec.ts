...
describe('DmaApiService', () => {
    ...
    it('should fully type an object to an Entity', () => {
        ...
        apiService.getResource<Spell>(`${environment.baseApiUrl}/api/spell/spell-2`, 'Spell').subscribe({
            next: (response) => {
                expect(response).toEqual(expectedSpell);
            },
        });

        httpTestingController.expectOne(`${environment.baseApiUrl}/api/spell/spell-2`).flush({
            id: 'spell-2',
            name: 'Aid',
            level: 2,
